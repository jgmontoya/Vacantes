...
class Vacantes_viewer(object):

    def course_info(self, data, soup):
    ...
    def vacancies_finder(self, data, final_data):
        for course in data:
            data_index = [6, 7, 8]
            course_data_list = []
            new_r = requests.get('http://buscacursos.uc.cl/informacionVacReserva.ajax.php?nrc=' + str(
                course["nrc"]) + '&termcode=2016-2&cantidad_dis=' + str(course["cantidad_dis"]) + '&cantidad_min=' + str(
                course["cantidad_total"]) + '&cantidad_ocu=' + str(
                int(course["cantidad_total"]) - int(course["cantidad_dis"])) + '&nombre=' + str(
                course["nombre"]) + '&sigla=' + str(course["sigla"]) + '&seccion=' + str(course["seccion"]))
...
for nrc in nrcs:
    ...
    V = Vacantes_viewer()
    V.course_info(data, soup)


V.vacancies_finder(data, final_data)
V.show_info(final_data)
plt.show()